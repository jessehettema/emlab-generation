    ...
    private class MarketInformation {
        ...
        MarketInformation(ElectricitySpotMarket market, Map<ElectricitySpotMarket, Double> expectedDemand,
            ...
            for (SegmentLoad segmentLoad : market.getLoadDurationCurve()) {
                ...
                // if demand is flexible set the expectedSegmentLoad to the new
                // peak demand value calculated in
                // clearTwoInterconnectedMarketsGivenAnInterconnectorAdjustedLoad
                if (reps.segmentLoadRepository.returnDemandElasticityTimeSeriesBySegmentAndMarket(segment,
                        segmentLoad.getElectricitySpotMarket())[(int) getCurrentTick()] != 0d) {
                    SegmentClearingPoint segmentClearingPoint = reps.segmentClearingPointRepository
                            .findOneSegmentClearingPointForMarketSegmentAndTime(getCurrentTick(), segment,
                                    segmentLoad.getElectricitySpotMarket(), false);
                    expectedSegmentLoad = segmentClearingPoint.getVolume() / segment.getLengthInHours();
                }
